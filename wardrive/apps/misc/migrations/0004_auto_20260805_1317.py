# Generated by Django 5.2 on 2026-08-05 19:17
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        django_db_views.operations.ViewRunPython(
            code=django_db_views.migration_functions.ForwardViewMigration("SELECT\n            lte.id,\n            lte.mcc,\n            lte.mnc,\n            lte.lac,\n            lte.cell_id,\n            lte.cell_type,\n            lte.state,\n            lte.enodeb_id,\n            lte.sector_id,\n            lte.pci,\n            lte.band,\n            lte.earfcn,\n            lte.dl_freq_mhz,\n            lte.ul_freq_mhz,\n            lte.rssi,\n            lte.rsrp,\n            lte.rsrq,\n            lte.sinr,\n            CASE\n                WHEN lte.rssi > -50 THEN 'Excellent'\n                WHEN lte.rssi BETWEEN -60 AND -50 THEN 'Good'\n                WHEN lte.rssi BETWEEN -70 AND -60 THEN 'Fair'\n                ELSE 'Weak'\n            END AS signal_streng,\n            lte.provider,\n            lte.tech,\n            lte.first_seen,\n            lte.device_source,\n            lte.uploaded_by,\n            lte.current_latitude,\n            lte.current_longitude,\n            COALESCE(city.city, 'Unknown') AS city,\n            COALESCE(city.country, 'Unknown') AS country,\n            COALESCE(city.country_iso, 'ZZ') AS country_iso\n        FROM lte_wardriving AS lte\n        LEFT JOIN LATERAL (\n            SELECT gc.city, gc.country, gc.country_iso\n            FROM geos_city gc\n            WHERE gc.deleted_at IS NULL\n                AND gc.polygon && lte.location\n                AND ST_Intersects(gc.polygon, lte.location)\n            LIMIT 1\n        ) AS city ON TRUE\n        WHERE\n            (lte.current_latitude != 0 AND lte.current_longitude != 0)\n            AND lte.deleted_at IS NULL\n            AND lte.location IS NOT NULL", 'wardriving_mobile', engine='django.db.backends.postgresql'),
            reverse_code=django_db_views.migration_functions.BackwardViewMigration('', 'wardriving_mobile', engine='django.db.backends.postgresql'),
            atomic=False,
        ),
        django_db_views.operations.ViewRunPython(
            code=django_db_views.migration_functions.ForwardViewMigration('SELECT 1 AS id WHERE 1=0', 'wardriving_mobile', engine='django.db.backends.sqlite3'),
            reverse_code=django_db_views.migration_functions.BackwardViewMigration('', 'wardriving_mobile', engine='django.db.backends.sqlite3'),
            atomic=False,
        ),
        django_db_views.operations.ViewRunPython(
            code=django_db_views.migration_functions.ForwardViewMigration('SELECT 1 AS id WHERE 1=0', 'wardriving_mobile', engine='django.db.backends.mysql'),
            reverse_code=django_db_views.migration_functions.BackwardViewMigration('', 'wardriving_mobile', engine='django.db.backends.mysql'),
            atomic=False,
        ),
    ]
